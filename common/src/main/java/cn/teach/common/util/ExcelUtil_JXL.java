...
public class ExcelUtil_JXL {
    ...
    public static String exportExcels(String fileName, List<String> workbookNameList, List<Map<String, Object>> exportMapList,
                                      List<List<Map<String, Object>>> dataList, HttpServletResponse response, HttpServletRequest request) {
        String result = "SUCCESS";
        try {
            OutputStream os = response.getOutputStream();// 取得输出流
            response.reset();// 清空输出流
            ...
            response.addCookie(cookie);
            WritableWorkbook workbook = Workbook.createWorkbook(os);
            for (int k = 0; k < workbookNameList.size(); k++) {
                String workbookName = workbookNameList.get(k);
                Map exportMap = exportMapList.get(k);
                List<Map<String, Object>> list= dataList.get(k);
                WritableSheet sheet = workbook.createSheet(workbookName, k);
                jxl.SheetSettings sheetset = sheet.getSettings();
                sheetset.setProtected(false);
                WritableFont NormalFont = new WritableFont(WritableFont.ARIAL, 10);
                WritableFont BoldFont = new WritableFont(WritableFont.ARIAL, 10, WritableFont.BOLD);
